...
export default function MyPage() {
  ...
  const handleChangeStatus = async (activityId, newStatus) => {
    ...
    if (
      confirm(
        `정말로 이 활동의 모집 상태를 '${newStatusLabel}'(으)로 변경하시겠습니까?`
      )
    ) {
      try {
  ...
  const handleDeleteActivity = async (activityId, status) => {
    ...
    if (
      confirm(
        `정말로 이 활동을 ${actionText}하시겠습니까? 이 작업은 되돌릴 수 없습니다.`
      )
    ) {
      try {
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handlePhoneChange} // 기존 인라인 함수를 handlePhoneChange로 변경
