...
export default function MyPage() {
  ...
  const handleRemoveApplicant = (applicantId) => {
    if (confirm('이 신청자를 삭제하시겠습니까?')) {
      setActivitiesData(prevData =>
        prevData.map(activity =>
          activity.id === selectedActivity.id
            ? {
                ...activity,
                applicantsList: activity.applicantsList.filter(app => app.id !== applicantId),
                applicants: activity.applicants - 1
              }
            : activity
        )
      );
      ...
      setSelectedActivity(prev => ({
        ...prev,
        applicantsList: prev.applicantsList.filter(app => app.id !== applicantId),
  ...
  const handleAddInterest = () => {
    const interest = prompt('새로운 관심분야를 입력하세요:');
    ...
    <div className="min-h-screen bg-gray-50 pb-20">
                    ...
                    <button
                      onClick={(e) => {
                        if (attendanceOpen[activity.id]) {
                          ...
                          handleOpenAttendance(e, activity);
                        }
                      }}
                      className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${
                        ...
                        <div className="mt-3">
                          <div className="flex items-center justify-between mb-2">
                            ...
                            <span className="text-sm text-gray-500">
                              {Math.round((activity.currentMembers / activity.maxMembers) * 100)}%
                            </span>
                          </div>
                    ...
                    {activity.canCancel && (
                      <button
                        onClick={(e) => {
                          e.preventDefault();
                          e.stopPropagation();
                          if (confirm('정말로 이 활동 신청을 취소하시겠습니까?')) {
      ...
      {showProfileEditModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl w-full max-w-md max-h-[75vh] overflow-hidden flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="block text-sm font-medium text-gray-700">관심분야</label>
                  <button
                    ...
                    <div key={applicant.id} className="bg-gray-50 rounded-xl p-4 border border-gray-100">
                      ...
                      <div className="grid grid-cols-2 gap-3 text-sm">
                        <div>
                          ...
                          <span className="ml-2 text-gray-900">{applicant.studentId}</span>
                        </div>
                        <div>
                          <span className="text-gray-500">학과:</span>
                          <span className="ml-2 text-gray-900">{applicant.department}</span>
                        </div>
                        <div className="col-span-2">
                          <span className="text-gray-500">신청일:</span>
                          <span className="ml-2 text-gray-900">{applicant.appliedDate}</span>
                        </div>
                      </div>
                    </div>
                  ))}
