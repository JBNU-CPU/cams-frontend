...
export default function MyPage() {
  ...
  useEffect(() => {
    const fetchMyPageData = async () => {
      ...
      try {
        ...
        const interestsArray = (typeof profileData.interests === 'string' && profileData.interests)
        ...
        setUserProfile({
          ...
          introduction: profileData.introduction || "자기소개를 작성해주세요.",
  ...
  const handleSaveProfile = () => { setUserProfile(editForm); setShowProfileEditModal(false); }; // 실제로는 API 호출 필요
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-500">마이페이지 정보를 불러오는 중...</p>
        </div>
      </div>
    );
  }
      ...
      >
        {isLoggedIn && (
          <div className="flex space-x-2">
            <button onClick={() => setActiveTab('profile')} className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === 'profile' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600'}`}>
              내 프로필
            </button>
            <button onClick={() => setActiveTab('opened')} className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === 'opened' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600'}`}>
              개설목록
            </button>
            <button onClick={() => setActiveTab('applied')} className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === 'applied' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600'}`}>
              신청목록
            </button>
          </div>
        )}
                ...
                <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="text-center p-4 bg-blue-50 rounded-lg">
                      <div className="text-2xl font-bold text-blue-600 mb-1">{myOpenedActivities.length}</div>
                            ...
                            <div className="mt-3">
                              <div className="flex items-center justify-between mb-2">
                                <div className="flex items-center text-sm text-gray-600">
                                  ...
                                  <span>참여 인원: {activity.members}/{activity.maxMembers}명</span>
                                </div>
                                <span className="text-sm text-gray-500">{Math.round((activity.members / activity.maxMembers) * 100)}%</span>
                              </div>
                              <div className="w-full bg-gray-200 rounded-full h-2">
                                <div className="bg-blue-600 h-2 rounded-full transition-all duration-300" style={{ width: `${(activity.members / activity.maxMembers) * 100}%` }}></div>
                              </div>
                            </div>
                          </div>
            ...
            <div className="px-6 py-3 bg-blue-50">
              <div className="flex items-center justify-center space-x-2">
                ...
                <span className="text-sm font-medium text-blue-700">
                  총 {selectedActivity.applicants}명의 신청자
                </span>
              </div>
            </div>
                  ...
                  {selectedActivity.applicantsList.map((applicant) => (
                    <div key={applicant.email} className="bg-gray-50 rounded-xl p-4 border border-gray-100">
                      <div className="flex items-start justify-between mb-3">
                        ...
                        {/* ❗❗ 핵심 수정 3: 삭제 핸들러에 applicant.id 전달 ❗❗ */}
                        <button onClick={() => handleRemoveApplicant(applicant.id)} className="w-8 h-8 flex items-center justify-center text-red-600 hover:bg-red-50 rounded-full transition-colors" > <i className="ri-delete-bin-line"></i> </button>
                      </div>
                      <div className="grid grid-cols-2 gap-3 text-sm">
                        <div>
                          <span className="text-gray-500">전화번호:</span>
                          <span className="ml-2 text-gray-900">{applicant.phone}</span>
                        </div>
                        <div className="col-span-2">
                          <span className="text-gray-500">참가일자:</span>
                          <span className="ml-2 text-gray-900">{applicant.joindAt ? applicant.joindAt.substring(0, 10) : '-'}</span>
                        </div>
                      </div>
                    </div>
                  ))}
