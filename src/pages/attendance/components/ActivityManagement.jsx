...
export default function ActivityManagement() {
  ...
  const [showAttendanceStatsModal, setShowAttendanceStatsModal] =
    useState(false);
  ...
  const [selectedMemberForSessionDetails, setSelectedMemberForSessionDetails] =
    useState(null);
  const [memberSessionAttendanceDetails, setMemberSessionAttendanceDetails] =
    useState([]);
  const [currentAttendanceModalView, setCurrentAttendanceModalView] =
    useState("overview"); // New state
  ...
  const fetchActivitiesAndStatuses = async () => {
    try {
      const activityResponse = await axiosInstance.get(
        "/api/me/activity/create"
      );
      if (
        !activityResponse.data ||
        !Array.isArray(activityResponse.data.content)
      ) {
        setActivityData([]);
        return;
      }
      ...
      for (const activity of activities) {
        try {
          const ongoingSessionResponse = await axiosInstance.get(
            `/api/session/activity/${activity.id}/ongoing`
          );
          ...
          if (ongoingSession && ongoingSession.status === "OPEN") {
            ...
            const sessionResponse = await axiosInstance.get(
              `/api/session/activity/${activity.id}`
            );
          ...
          console.error(
            `Error fetching session for activity ${activity.id}:`,
            error
          );
          buttonStatuses[activity.id] = "출석 오픈";
          openStates[activity.id] = false;
  ...
  useEffect(() => {
    ...
    if (
      ...
      currentOngoingSession[selectedActivity.id]
    ) {
      ...
      const calculateTimeLeft = async () => {
        ...
        const diffSeconds = Math.max(
          0,
          Math.floor((closedAt.getTime() - now.getTime()) / 1000)
        );
        ...
        if (diffSeconds <= 0) {
          ...
          try {
            await axiosInstance.put(`/api/session/${session.sessionId}`, {
              status: "CLOSED",
            });
          } catch (error) {
            console.error(
              "Error explicitly closing session in backend:",
              error
            );
            // Even if backend close fails, proceed with UI update to reflect timer end
          }

          setActivityButtonStatus((prev) => ({
            ...prev,
            [selectedActivity.id]: "출석 마감 됨",
          }));
        ...
        try {
          ...
          setAttendanceRate(
            attendanceData.length > 0
              ? Math.round(
                  ((presentMembers.length + lateMembers.length) /
  ...
  const handleStartAttendance = async () => {
    ...
    try {
      ...
      const ongoingSessionResponse = await axiosInstance.get(
        `/api/session/activity/${selectedActivity.id}/ongoing`
      );
      ...
      setCurrentOngoingSession((prev) => ({
        ...prev,
        [selectedActivity.id]: newOngoingSession,
      }));
  ...
  const handleCloseAttendance = async (activityId) => {
    try {
      ...
      if (!session || !session.sessionId) {
        console.error(
          "No ongoing session found for this activity or session ID is missing."
        );
      ...
      await axiosInstance.put(`/api/session/${sessionId}`, {
        status: "CLOSED",
      }); // PUT request
  ...
  const handleConfirmReopen = async () => {
    ...
    if (minutes != 1987 && (isNaN(minutes) || minutes < 1 || minutes > 120)) {
      setAlertMessage("출석 시간은 1분에서 120분 사이로 설정해주세요.");
    ...
    try {
      await axiosInstance.patch(
        `/api/session/${reopenSessionData.sessionId}/info`,
        {
  ...
  const handleAttendanceManagement = async (activity) => {
    ...
    try {
      const response = await axiosInstance.get(
        `/api/attendance/me/create/${activity.id}`
      );
      if (response.data) {
  ...
  const formatSchedule = (recurringSchedules, eventSchedules) => {
    ...
    if (recurringSchedules && recurringSchedules.length > 0) {
      ...
      const formattedSchedules = eventSchedules.map((e) => {
        ...
        const startTime = `${String(start.getHours()).padStart(
          2,
          "0"
  ...
  const handleEditActivity = (activity) => {
    ...
    if (activity.recurringSchedules && activity.recurringSchedules.length > 0) {
      parsedDays = activity.recurringSchedules
        .map((s) => {
          const dayName = Object.keys(daysMapReverse).find(
            (key) => daysMapReverse[key] === s.dayOfWeek
          );
          return dayName;
        })
        .filter(Boolean);
      ...
      parsedTime = `${String(startDate.getHours()).padStart(2, "0")}:${String(
  ...
  const handleViewMemberSessionDetails = async (member) => {
    ...
    try {
      ...
      const response = await axiosInstance.get(
        `/api/attendance/activity/${selectedActivity.id}/member/${member.id}`
      );
  ...
  const handleUpdateMemberSessionAttendance = async (
    attendanceId,
    newStatus
  ) => {
    try {
      await axiosInstance.patch(`/api/attendance/${attendanceId}`, {
        status: newStatus,
      });
      ...
      const response = await axiosInstance.get(
        `/api/attendance/activity/${selectedActivity.id}/member/${selectedMemberForSessionDetails.id}`
      );
      setMemberSessionAttendanceDetails(response.data);
  ...
  return (
    <div className="space-y-4">
      ...
      {(() => {
        const displayActivities = activityData.filter(
          (activity) =>
            activity.isApproved &&
              ...
              <div className="flex-1">
                ...
                <div className="space-y-1 text-sm text-gray-600">
                  <div className="flex items-center">
                    ...
                    <span>
                      멤버: {activity.participantCount}/
                      {activity.maxParticipants}명
                    </span>
                  </div>
                  <div className="flex items-center">
                    ...
                    <span>
                      {formatSchedule(
                        activity.recurringSchedules,
                        activity.eventSchedules
                      )}
                    </span>
                  </div>
                  <div className="flex items-center">
                      ...
                      {attendedMembers.length === 0 ? (
                        ...
                        <div className="space-y-3">
                          {attendedMembers.map((member) => (
                            <div
                              key={member.id}
                              className="bg-gray-50 rounded-xl p-4 hover-gray-100 transition-colors"
                            >
                              <div className="flex items-center justify-between">
                ...
                <div className="px-6 py-4 border-b border-gray-100">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex-1 text-center">
                      <h3 className="text-lg font-semibold text-gray-900">
                        {selectedMemberForSessionDetails?.name}님의 출결 상세
                      </h3>
                      <p className="text-sm text-gray-600">
                        {selectedActivity?.title}
                      </p>
                        ...
                        >
                          <div className="flex justify-between items-center mb-2">
                            <span className="font-medium text-gray-900">
                              {sessionDetail.sessionNumber}회차
                            </span>
                            <span className="text-sm text-gray-600">
                              {new Date(
                                sessionDetail.attendanceTime
                          ...
                          <div className="flex items-center space-x-2">
                            <label
                              htmlFor={`status-${sessionDetail.sessionId}`}
                            ...
                            <select
                              ...
                              onChange={(e) =>
                                handleUpdateMemberSessionAttendance(
                                  sessionDetail.attendanceId,
                                  e.target.value
                                )
                              }
            ...
            <div className="mb-6">
              ...
              <div className="flex justify-center space-x-2 mb-6">
                {[0, 1, 2, 3].map((index) => (
                  <div
                    key={index}
                    className="w-12 h-12 border-2 border-blue-200 rounded-lg flex items-center justify-center"
                  >
              ...
              <div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {["18:00", "18:30", "19:00", "19:30", "20:00", "20:30"].map(
                      (time) => (
                        <button
                          key={time}
                          type="button"
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md max-h-[85vh] overflow-y-auto flex flex-col">
            <div className="flex items-center justify-between mb-6">
              ...
              <button
                onClick={() => setShowAttendanceStatsModal(false)}
                className="p-1 text-gray-400 hover:text-gray-600"
              >
                <i className="ri-close-line text-xl"></i>
              </button>
                    ...
                    {participantSummaries.map((member, index) => (
                      ...
                      >
                        <div className="flex justify-between items-center mb-2">
                          ...
                          <span
                            className={`font-bold text-lg ${getAttendanceRateColor(
                        ...
                        <button
                          onClick={() =>
                            handleViewMemberSessionDetails({
                              ...member,
